...
class Blip2Llama(Blip2Base):
    ...
    Supported model types:
        - blip2_llama: pretrained model with llama7b
    Usage:
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            inputs_query = self.llm_proj(query_output.last_hidden_state)
            atts_query = torch.ones(inputs_query.size()[:-1], dtype=torch.long).to(
                image.device
            )
            ...
            llm_tokens = self.llm_tokenizer(prompt, return_tensors="pt").to(
                image.device
            )
            ...
            if use_nucleus_sampling:
                query_embeds = inputs_query.repeat_interleave(num_captions, dim=0)
                num_beams = 1
            else:
            ...
            prompt_length = llm_tokens.input_ids.shape[1]
