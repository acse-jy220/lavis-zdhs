...
from lavis.models.blip2_models.modeling_baichuan_7b import BaiChuanForCausalLM

@registry.register_model("blip2_baichuan7b_instruct")
class Blip2Baichuan7BInstruct(Blip2Base):
    """
    BLIP2 model.
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "baichuan7b": "configs/models/zdtc/blip2_instruct_baichuan7b.yaml",
        "baichuan7b_server": "configs/models/zdtc/blip2_instruct_baichuan7b_server.yaml",
        "baichuan7b_stage2": "configs/models/zdtc/blip2_baichuan7b.yaml",
    }
    ...
    def __init__(
        ...
        assert transformers_version >= version.parse("4.29"), "requires transformers>=4.29"        
        ...
        self.llm_model = BaiChuanForCausalLM.from_pretrained(llm_model, torch_dtype=torch.float16, trust_remote_code=True)
        ...
        if(peft_type):
            ...
            # assert not (peft_type and peft_model), "peft_type and peft_model cannot exist at the same time"
        if(peft_type == 'lora'):
            peft_config = LoraConfig(
                task_type=TaskType.CAUSAL_LM, inference_mode=False, r=8, lora_alpha=32, lora_dropout=0.1, target_modules=['W_pack'], 
            )
            self.llm_model = get_peft_model(self.llm_model, peft_config)
            logging.info("build lora finetune model")
    ...
    def forward(self, samples):
        ...
        samples_text_input = [samples['context'][i] + samples_text_input[i] for i in range(len(samples_text_input))]
    ...
    def stream_generator(
        ...
        with self.maybe_autocast():
            ...
            streamer = TextIteratorStreamer(self.llm_tokenizer, skip_prompt=True)
    ...
    def predict_class(
        ...
        if type(candidates[0]) == list:
            ...
            return (self._predict_class(samples, candidates, n_segments)).tolist()
    ...
    def from_config(cls, cfg):
        ...
        if qformer_text_input and cfg.get("add_chinese_tokens", False):
            model.add_tokens(cfg.get("chinese_tokens_path","data/bert-base-chinese-vocab-clean.txt"))
    ...
    def add_tokens(self, tokens_path):
        new_tokenizer = BertTokenizer.from_pretrained(tokens_path)
        added = self.tokenizer.add_tokens(list(new_tokenizer.get_vocab().keys()))
        self.Qformer.resize_token_embeddings(len(self.tokenizer))
        logging.info("added tokens from: {}".format(tokens_path))
        logging.info("added tokens number: {}".format(added))